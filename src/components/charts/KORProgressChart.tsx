...
export function KORProgressChart({ currentRevenue, currency = '€' }: KORProgressChartProps) {
  const formatCurrency = (value: number) => {
    return `${currency}${value.toLocaleString('nl-NL', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
  };

  const percentageToLower = Math.min((currentRevenue / KOR_THRESHOLDS.LOWER) * 100, 100);
  const percentageToUpper = Math.min((currentRevenue / KOR_THRESHOLDS.UPPER) * 100, 100);
  ...
  return (
    <Card className="col-span-full">
      <CardHeader>
        ...
        <CardDescription>
          Wykorzystanie progów zwolnienia z VAT dla małych przedsiębiorstw w Holandii
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Status aktualny */}
        <div className={`${getZoneColor()} text-black p-6 rounded-xl`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold mb-1 opacity-90">OBECNY OBRÓT ROCZNY</p>
              <p className="text-4xl font-black">{formatCurrency(currentRevenue)}</p>
              <p className="text-sm mt-2 opacity-90">{getZoneText()}</p>
            </div>
            {isAboveLower && !isAboveUpper ? (
              <CheckCircle size={64} weight="duotone" className="opacity-80" />
            ) : isAboveUpper ? (
              <Warning size={64} weight="duotone" className="opacity-80" />
            ) : (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${isAboveLower ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
              <span className="font-semibold text-black dark:text-white">Próg dolny KOR</span>
            </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${isAboveUpper ? 'bg-red-500' : isNearUpper ? 'bg-orange-500' : 'bg-slate-300'}`}></div>
              <span className="font-semibold text-black dark:text-white">Próg górny KOR (utrata korzyści)</span>
            </div>
            <span className="font-mono font-bold text-black dark:text-white">
          ...
          <div className="flex items-center justify-between text-xs text-black dark:text-white">
            ...
            {!isAboveUpper && (
              <span className={`font-semibold ${isNearUpper ? 'text-orange-600 dark:text-orange-400' : 'text-black dark:text-white'}`}>
        ...
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          ...
          <p className="text-xs text-black dark:text-white leading-relaxed">
            Kleineondernemersregeling (KOR) to holenderski system zwolnienia z VAT dla małych firm. 
            Gdy obrót przekracza €{KOR_THRESHOLDS.LOWER.toLocaleString()}, możesz korzystać z korzyści KOR. 
            Przekroczenie €{KOR_THRESHOLDS.UPPER.toLocaleString()} oznacza utratę niektórych przywilejów.
          </p>
        </div>
      </CardContent>
    </Card>
