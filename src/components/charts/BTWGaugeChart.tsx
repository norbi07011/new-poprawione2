...
export function BTWGaugeChart({ vatRate, recommendedRate = 21, currency = '€' }: BTWGaugeChartProps) {
  // Clamp between 0-21%
      ...
      <CardContent className="space-y-6">
        ...
        <div className="relative w-full aspect-[2/1] flex items-end justify-center">
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center pb-4">
            ...
            <div className="text-xs text-black dark:text-white font-semibold mt-1">
              {getZoneLabel()}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-6">
          <div className="bg-blue-50 dark:bg-blue-950/30 p-3 rounded-lg text-center">
            ...
            <div className="text-lg font-bold text-black dark:text-white">0-9%</div>
          </div>
          <div className="bg-orange-50 dark:bg-orange-950/30 p-3 rounded-lg text-center">
            <div className="text-xs text-black dark:text-white font-semibold mb-1">
              Pośrednia
            </div>
            <div className="text-lg font-bold text-black dark:text-white">9-21%</div>
          </div>
          <div className="bg-emerald-50 dark:bg-emerald-950/30 p-3 rounded-lg text-center">
            <div className="text-xs text-black dark:text-white font-semibold mb-1">
              Standardowa
            </div>
            <div className="text-lg font-bold text-black dark:text-white">21%</div>
          </div>
        </div>
        ...
        <div className={`flex items-center gap-3 p-4 rounded-lg ${
          ...
          {isCorrectRate ? (
            <>
              <CheckCircle size={24} weight="duotone" className="text-black dark:text-white flex-shrink-0" />
              <div>
            ...
            <>
              <Warning size={24} weight="duotone" className="text-black dark:text-white flex-shrink-0" />
              <div>
                <p className="text-sm font-semibold text-black dark:text-white">
                  Sprawdź stawkę VAT
                </p>
                <p className="text-xs text-black dark:text-white mt-0.5">
                  Zalecana stawka dla branży budowlanej: {recommendedRate}% (obecnie: {clampedRate}%)
                </p>
              </div>
            </>
          )}
