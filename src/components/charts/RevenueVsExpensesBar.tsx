...
export function RevenueVsExpensesBar({ data, currency = '€' }: RevenueVsExpensesBarProps) {
        ...
        <div className="bg-white dark:bg-black/80 p-4 rounded-lg shadow-lg border border-slate-200 dark:border-blue-500/30">
          ...
          <div className="space-y-1 text-sm">
            <p className="flex items-center gap-2">
              ...
              <span className="text-black dark:text-white">Przychody:</span>
              <span className="font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(data.revenue)}</span>
            </p>
            <p className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-rose-500"></span>
              <span className="text-black dark:text-white">Wydatki:</span>
              <span className="font-bold text-rose-600 dark:text-rose-400">{formatCurrency(data.expenses)}</span>
            </p>
            <p className="flex items-center gap-2 pt-1 border-t border-slate-200 dark:border-blue-500/30">
              <span className="w-3 h-3 rounded-full bg-blue-500"></span>
              <span className="text-black dark:text-white">Zysk:</span>
              <span className={`font-bold ${data.profit >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400'}`}>
