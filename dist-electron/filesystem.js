...
class FileSystemService {
    ...
    async saveCompanyLogo(filename, data) {
        try {
            // Sanitize filename
            const sanitizedFilename = this.sanitizeFilename(filename);
            const filePath = path_1.default.join(this.documentsPath, 'logo', sanitizedFilename);
            // Ensure logo directory exists
            const logoDir = path_1.default.join(this.documentsPath, 'logo');
    ...
    async getCompanyLogoPath(filename) {
        const sanitizedFilename = this.sanitizeFilename(filename);
        const filePath = path_1.default.join(this.documentsPath, 'logo', sanitizedFilename);
