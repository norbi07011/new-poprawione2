    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white dark:bg-black/80 p-4 rounded-lg shadow-lg border border-slate-200 dark:border-blue-500/30">
          <p className="font-bold text-black dark:text-white mb-2">{data.month}</p>
          <div className="space-y-1 text-sm">
            <p className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-rose-500"></span>
              <span className="text-black dark:text-white">VAT należny:</span>
              <span className="font-bold text-rose-600 dark:text-rose-400">{formatCurrency(data.vatOwed)}</span>
            </p>
            <p className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-emerald-500"></span>
              <span className="text-black dark:text-white">VAT naliczony:</span>
              <span className="font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(data.vatPaid)}</span>
            </p>
            <p className="flex items-center gap-2 pt-1 border-t border-slate-200 dark:border-blue-500/30">
              <span className="w-3 h-3 rounded-full bg-blue-500"></span>
              <span className="text-black dark:text-white">Saldo:</span>
              <span className={`font-bold ${data.balance >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-orange-600 dark:text-orange-400'}`}>
                {formatCurrency(Math.abs(data.balance))} {data.balance < 0 ? '(zwrot)' : '(wpłata)'}
              </span>
            </p>
          </div>
        </div>
