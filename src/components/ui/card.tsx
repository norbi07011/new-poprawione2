...
function Card({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "premium-card relative backdrop-blur-md text-black dark:text-white flex flex-col gap-6 py-6",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-none font-bold text-xl text-black dark:text-white", className)}
      {...props}
    />
  )
}
